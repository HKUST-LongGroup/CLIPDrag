    ...
    for step_idx in range(args.n_pix_step):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            if step_idx != 0:
                ...
                # handle_points = point_tracking_a(F0, F1, handle_points, handle_points_init, target_points,args)
...
def drag_diffusion_update_a(model,
                          init_code,
                          text_embeddings,
                          t,
                          handle_points,
    ...
    for step_idx in range(args.n_pix_step):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            if step_idx != 0:
                if args.lam == 0:
                    print('using default point tracking')
                    handle_points = point_tracking(F0, F1, handle_points, handle_points_init, args)
                else :
                    print('using distance-awareness point tracking.')
                    handle_points = point_tracking_a(F0, F1, handle_points, handle_points_init,target_points, args)
                print('new handle points', handle_points)
    ...
    return init_code
def drag_diffusion_update_b(model,
                          init_code,
                          text_embeddings,
                          t,
                          handle_points,
...
def drag_diffusion_update_good(model,
                          init_code,
                          text_embeddings,
                          t,
                          handle_points,
    ...
    for step_idx in range(args.n_pix_step):
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            for i in range(len(handle_points)):
                ...
                # f0_patch = F1[:,:,int(pi[0])-args.r_m:int(pi[0])+args.r_m+1, int(pi[1])-args.r_m:int(pi[1])+args.r_m+1].detach()
                # f1_patch = interpolate_feature_patch(F1, pi[0] + di[0], pi[1] + di[1], args.r_m)
                loss += ((2*args.r_m)**2)*F.l1_loss(f0_patch, f1_patch)

            # masked region must stay unchanged
            print(f'Loss from drag: {loss}')
            if using_mask:
                loss += args.lam * ((x_prev_updated-x_prev_0)*(1.0-interp_mask)).abs().sum()
            # loss += args.lam * ((init_code_orig-init_code)*(1.0-interp_mask)).abs().sum()
            print('loss total=%f'%(loss.item()))
    ...
    return init_code
def drag_diffusion_update_final(model,
                          init_code,
                          text_embeddings,
                          t,
                          handle_points,
