...
from .drag_utils import drag_diffusion_update, drag_diffusion_update_gen,drag_diffusion_update_final,drag_diffusion_update_a,drag_diffusion_update_b,drag_diffusion_update_good
...
def run_drag(source_image,
    ...
    print('run drag: baseline method')
    # initialize model
...
def run_drag_a(source_image,
    ...
    print('run drag_a: modify point tracking method.')
    # initialize model
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    scheduler = DDIMScheduler(beta_start=0.00085, beta_end=0.012,
                          beta_schedule="scaled_linear", clip_sample=False,
    ...
    updated_init_code = drag_diffusion_update_a(
        model,
        init_code,
        text_embeddings,
        t,
...
def run_drag_b(source_image,
    ...
    print('run drag_b: based on baseline, clip guidance part.')
    # initialize model
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    scheduler = DDIMScheduler(beta_start=0.00085, beta_end=0.012,
                          beta_schedule="scaled_linear", clip_sample=False,
    ...
    updated_init_code = drag_diffusion_update_b(
        model,
        init_code,
        text_embeddings,
        t,
...
def run_drag_final(source_image,
             image_with_clicks,
             mask,
             prompt,
             points,
...
def run_drag_good(source_image,
             image_with_clicks,
             mask,
             prompt,
             points,
    ...
    # initialize model
    print('Run drag good: goodDrag --- change the motion supervision template in DragDiff. ')
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    scheduler = DDIMScheduler(beta_start=0.00085, beta_end=0.012,
                          beta_schedule="scaled_linear", clip_sample=False,
                          set_alpha_to_one=False, steps_offset=1)
    ...
    updated_init_code = drag_diffusion_update_good(
        model,
        init_code,
        text_embeddings,
        t,
...
def run_drag_final(source_image,
    ...
    print('run drag_final: our final method -- combine 3 parts above.')
    # initialize model
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    scheduler = DDIMScheduler(beta_start=0.00085, beta_end=0.012,
                          beta_schedule="scaled_linear", clip_sample=False,
    ...
    model.unet = model.unet.float()
    model.vae = model.vae.float()
    updated_init_code = drag_diffusion_update_final(
        model,
        init_code,
        text_embeddings,
        t,
